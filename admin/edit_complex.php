...
function getComplexInfo($db) {
    $id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
    if (!$id) return null;
    
    $stmt = $db->prepare("SELECT * FROM residential_complexes WHERE id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        $error = "Ошибка при обновлении информации: " . $stmt->error;
    }
}
?>
